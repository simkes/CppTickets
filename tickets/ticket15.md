...
* По умолчанию - `new Foo[n]{val1, val2, val3}` - неинициализированные инициализируются по умолчанию.
...
```c++
int main() {
    std::vector<int> const& first = std::vectorP{0, 0, 0};
    std::cout << first.size() << std::endl; // NO UB
    std::vector<int> const& second = std::vectorP{0, 0, 0}; // would have been dangling if lifetime of second had been wider than lifetime of first;     
}
```
...
int main() {
    std::vector<int> const& first = func(1);
    std::cout << first; // UB
}
```
...
```c++
int main() {
    auto const& val = std::min(0, 1); // Dangling reference
}
```
